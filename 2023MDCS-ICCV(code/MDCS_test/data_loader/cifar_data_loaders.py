...
class ImbalanceCIFAR100DataLoader(DataLoader):
    ...
    """
    def __init__(self, data_dir, batch_size, distribution = False, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True, imb_type='exp', imb_factor=0.01):
        augmentation_strong = [
        ...
        if training:
            dataset = IMBALANCECIFAR100(data_dir, train=True, download=True, transform=[transforms.Compose(augmentation_weak),transforms.Compose(augmentation_strong)], imb_type=imb_type, imb_factor=imb_factor)
        ...
        if balanced:
            ...
            if distribution:
                if training:
                    #pdb.set_trace()
                    sampler = DistributionSampler(dataset)
            else:
                sampler = None
...
class ImbalanceCIFAR10DataLoader(DataLoader):
    ...
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=1, training=True, balanced=False, retain_epoch_size=True, imb_factor=0.01):
        ...
        if training:
            print(imb_factor)
            dataset = IMBALANCECIFAR10(data_dir, train=True, download=True, transform=[transforms.Compose(c),transforms.Compose(augmentation_regular)], imb_factor=imb_factor)
