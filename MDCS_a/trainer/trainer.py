...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, criterion, metric_ftns, optimizer, config, data_loader,
        ...
        #self.start_epoch = 60
        self.max_val_accuracy = 0  # 初始化最大验证准确率
    ...
    def _train_epoch(self, epoch):
        ...
        if self.do_validation:
            ...
            # Add MAX_VAL_ACCURACY
            # 获取当前轮次的验证结果
            val_result = self.valid_metrics.result()
            # print("val_result:{}".format(val_result))

            # 更新最大验证准确率
            current_val_accuracy = val_result.get('accuracy', 0)
            if current_val_accuracy > self.max_val_accuracy:
                self.max_val_accuracy = current_val_accuracy
            # 输出最大的验证准确率
            # print(f"Maximum validation accuracy: {self.max_val_accuracy:.4f}")
